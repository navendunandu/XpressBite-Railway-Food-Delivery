...
if(isset($_POST['btn_submit'])) {
	...
	$checkQry = "select * from tbl_place where lower(place_name) = lower('$place') and district_id = '$district_id' and place_id != $eid";
	$checkResult = $con->query($checkQry);

	if($checkResult->num_rows > 0) {
		//If Place Already Exists
		echo " <script>
				alert('Unsuccessful, Place Already Exists...');
				window.location = 'Place.php';
			   </script> ";
	} else {
		//If no duplicate values found, proceed with insertion or updation
		if($eid == 0) {
